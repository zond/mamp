// precompute_filter.wgsl — Pre-compute steerable filter values at init time
//
// Runs ONCE per filter at pipeline creation. Writes filter h (or h² if
// squared=1) to a storage buffer, eliminating per-frame trig/log2/pow.
...
struct Params {
    ...
    squared: u32,     // 0=store h, 1=store h*h
}

@group(0) @binding(0) var<uniform> params: Params;
@group(0) @binding(1) var<storage, read_write> filter_out: array<f32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if params.squared == 1u {
        filter_val = filter_val * filter_val;
    }

    filter_out[idx] = filter_val;
}
