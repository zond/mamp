// fft_filter_store_accum.wgsl — 1D FFT with filter multiply + accumulate on store
//
// Same as fft.wgsl but during write-back, multiplies each output value by a
// pre-computed filter and accumulates (+=) into the output buffers. This fuses
// the apply_filter_accum dispatch into the FFT column pass, saving one dispatch
// per sub-band.
//
...
fn main(
    ...
    // Load with bit-reversal permutation (identical to fft.wgsl)
    ...
    // Write output: multiply by filter and accumulate
    ...
    for (var e = 0u; e < elems_per_thread; e++) {
        ...
        if dst_idx < n {
            ...
            accum_re[addr] += shared_re[dst_idx] * scale * h;
            accum_im[addr] += shared_im[dst_idx] * scale * h;
        }
    }
}
