...
#[allow(dead_code)] // Fields held for GPU bind group lifetimes
...
impl EvmPipeline {
    pub fn new(ctx: &GpuContext, w: u32, h: u32) -> Self {
        ...
        let buf_output_rgba = ctx.create_buffer(
            "evm_out",
            pixels * 4,
        ...
        let gaussian: [Buffer; N_LEVELS] = std::array::from_fn(|i| {
            ctx.create_buffer(
                &format!("g{}", i),
                3 * (level_w[i] as u64) * (level_h[i] as u64) * 4,
                s,
            )
        });
        ...
        log::info!(
            "Surface: {}x{}, format={:?}, alpha={:?}, present={:?}",
            w,
            h,
            surface_format,
            alpha_mode,
        ...
        let laplacian_temporal_pipeline =
            ctx.device
                .create_compute_pipeline(&ComputePipelineDescriptor {
                    label: Some("evm_lt"),
                    layout: None,
        ...
        let downsample_param_bufs: [Buffer; N_LEVELS - 1] = std::array::from_fn(|i| {
            ctx.create_uniform(
                ...
                &DownsampleParams {
                    in_width: level_w[i],
                    in_height: level_h[i],
                    out_width: level_w[i + 1],
                    out_height: level_h[i + 1],
                },
            )
        });
        ...
        let upsample_add_param_bufs: [Buffer; N_LEVELS - 1] = std::array::from_fn(|i| {
            ctx.create_uniform(
                &format!("ua_p{}", i),
                &UpsampleAddParams {
                    fine_width: level_w[i],
                    fine_height: level_h[i],
                    coarse_width: level_w[i + 1],
                    coarse_height: level_h[i + 1],
                },
            )
        });
        ...
        let rgba_to_chw_bg = ctx.device.create_bind_group(&BindGroupDescriptor {
            ...
            entries: &[
                BindGroupEntry {
                    binding: 0,
                    resource: frame_param_buf.as_entire_binding(),
                },
                BindGroupEntry {
                    binding: 1,
                    resource: buf_frame_rgba.as_entire_binding(),
                },
                BindGroupEntry {
                    binding: 2,
                    resource: gaussian[0].as_entire_binding(),
                },
            ],
        });
        ...
        let chw_to_rgba_bg = ctx.device.create_bind_group(&BindGroupDescriptor {
            ...
            entries: &[
                ...
                BindGroupEntry {
                    binding: 1,
                    resource: recon[0].as_entire_binding(),
                },
                BindGroupEntry {
                    binding: 2,
                    resource: buf_output_rgba.as_entire_binding(),
                },
            ],
        });
        ...
        let blit_bg = ctx.device.create_bind_group(&BindGroupDescriptor {
            ...
            entries: &[
                BindGroupEntry {
                    binding: 0,
                    resource: blit_param_buf.as_entire_binding(),
                },
                BindGroupEntry {
                    binding: 1,
                    resource: buf_output_rgba.as_entire_binding(),
        ...
        let downsample_bgs: [BindGroup; N_LEVELS - 1] = std::array::from_fn(|i| {
            ctx.device.create_bind_group(&BindGroupDescriptor {
                ...
                entries: &[
                    BindGroupEntry {
                        binding: 0,
                        resource: downsample_param_bufs[i].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 1,
                        resource: gaussian[i].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 2,
                        resource: gaussian[i + 1].as_entire_binding(),
                    },
                ],
            })
        });
        ...
        let laplacian_temporal_bgs: [BindGroup; N_LEVELS] = std::array::from_fn(|i| {
            ...
            ctx.device.create_bind_group(&BindGroupDescriptor {
                ...
                entries: &[
                    BindGroupEntry {
                        binding: 0,
                        resource: level_param_bufs[i].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 1,
                        resource: gaussian[i].as_entire_binding(),
                    ...
                    BindGroupEntry {
                        binding: 3,
                        resource: lp_high[i].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 4,
                        resource: lp_low[i].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 5,
                        resource: amplified[i].as_entire_binding(),
                    },
                ],
            })
        });
        ...
        let upsample_add_bgs: [BindGroup; N_LEVELS - 1] = std::array::from_fn(|i| {
            ctx.device.create_bind_group(&BindGroupDescriptor {
                ...
                entries: &[
                    BindGroupEntry {
                        binding: 0,
                        resource: upsample_add_param_bufs[i].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 1,
                        resource: amplified[i].as_entire_binding(),
                    ...
                    BindGroupEntry {
                        binding: 2,
                        resource: recon[i + 1].as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 3,
                        resource: recon[i].as_entire_binding(),
                    },
                ],
            })
        });
        ...
        Self {
            ...
            frame_param_buf,
            blit_param_buf,
            downsample_param_bufs,
            level_param_bufs,
            upsample_add_param_bufs,
            rgba_to_chw_bg,
            chw_to_rgba_bg,
            blit_bg,
            downsample_bgs,
            laplacian_temporal_bgs,
            upsample_add_bgs,
        }
    }
    ...
    pub fn process_and_render(
        ...
        ctx.queue
            .write_buffer(&self.buf_frame_rgba, 0, bytemuck::cast_slice(frame_rgba));

        for i in 0..N_LEVELS {
            let p = LevelParams {
                width: self.level_w[i],
                height: self.level_h[i],
                coarse_width: if i + 1 < N_LEVELS {
                    self.level_w[i + 1]
                } else {
                    0
                },
                coarse_height: if i + 1 < N_LEVELS {
                    self.level_h[i + 1]
                } else {
                    0
                },
            ...
            ctx.queue
                .write_buffer(&self.level_param_bufs[i], 0, bytemuck::bytes_of(&p));
        }
        ...
        let mut encoder = ctx
            .device
            .create_command_encoder(&CommandEncoderDescriptor { label: Some("evm") });
        ...
        Self::dispatch(
            &mut encoder,
            "r2c",
            &self.rgba_to_chw_pipeline,
            &self.rgba_to_chw_bg,
            GpuContext::div_ceil(self.width, 64),
            GpuContext::div_ceil(self.height, 16),
            1,
        );
        ...
        for i in 0..(N_LEVELS - 1) {
            Self::dispatch(
                ...
                &self.downsample_bgs[i],
                GpuContext::div_ceil(self.level_w[i + 1], 16),
                GpuContext::div_ceil(self.level_h[i + 1], 16),
                1,
            );
        }
        ...
        for i in 0..N_LEVELS {
            Self::dispatch(
                &mut encoder,
                "lt",
                &self.laplacian_temporal_pipeline,
                &self.laplacian_temporal_bgs[i],
                GpuContext::div_ceil(self.level_w[i], 16),
                GpuContext::div_ceil(self.level_h[i], 16),
                1,
            );
        }
        ...
        encoder.copy_buffer_to_buffer(
            &self.amplified[N_LEVELS - 1],
            0,
            &self.recon[N_LEVELS - 1],
            0,
        ...
        for i in (0..(N_LEVELS - 1)).rev() {
            Self::dispatch(
                &mut encoder,
                "ua",
                &self.upsample_add_pipeline,
                &self.upsample_add_bgs[i],
                GpuContext::div_ceil(self.level_w[i], 16),
                GpuContext::div_ceil(self.level_h[i], 16),
                1,
            );
